/******************************************************
  ...
  significant portion of code written by Ramesh Krishnamurti
  major modifications to bade code done by Justin Liu
...
public:
   ...
   Heap( const long double * Priorities, const long double * Elements, int s, int c); 
   ...
   long double peekMax() const { return A[0].element ;} // Peek at minimum priority element.
   long double peekMaxPriority() const { return A[0].priority ;} // Peek at minimum priority.

   // Modifiers
   void insert( long double element, long double priority ); // Insert the pair <element,priority>.
   long double extractMax(); // Remove and return the highest (minimum) priority element.
...
Heap::Heap( const long double * Priorities, const long double * Elements, int s, int c){ 
   hCapacity = s + c;
   A = new Pair[hCapacity];
   hSize = 0;
   for (int i=0; i<s; i++) {
...
void Heap::insert(long double element, long double priority){
   //double capacity if size of heap == capacity
   if (hSize >= hCapacity) {
      Pair* newA = new Pair[hCapacity*2];
...
// (extractMax() calls trickleDown(0)).
   ...
   if (i >= maxindex || i < 0) {
      return;
   	  ...
   	  if (leftchild <= maxindex) {
   	  	  ...
   	  	  if (A[greaterchild].priority > A[i].priority) {
   	  	  	  swap(greaterchild,i);
